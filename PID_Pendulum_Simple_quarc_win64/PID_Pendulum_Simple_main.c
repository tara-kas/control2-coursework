...
typedef RT_MODEL_PID_Pendulum_Simple_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * PID_Pendulum_Simple(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(PID_Pendulum_Simple_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(PID_Pendulum_Simple_DW.HILReadTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: PID_Pendulum_Simple -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(PID_Pendulum_Simple_DW.HILReadTimebase_Task,
        (t_clock) PID_Pendulum_Simple_P.HILReadTimebase_Clock, 1000.0, -1)
        ;
      if (result == 0) {
